...
{
    private static readonly ItemTable instance = new ItemTable();
    public static ItemTable Instance => instance;
    ...
    public readonly Dictionary<int, Item> ItemDic = new Dictionary<int, Item>()
    {
        ...
        {
            8,
            new Item(8, "파괴의 유전자", ItemType.Weapon,
                new List<Stat> { new Stat(StatType.Attack, 100), new Stat(StatType.MaxHp, -40) },
        ...
        {
            10,
            new Item(10, "실프 스카프", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 5), new Stat(StatType.CriticalChance, 2) },
        ...
        {
            14,
            new Item(14, "저주 부적", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 40), new Stat(StatType.MaxHp, -15) },
                "누군가의 염원이 담긴 불길한 부적이다.", 2500)
        },
        {
            15,
            new Item(15, "검은 안경", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 20), new Stat(StatType.MaxHp, 100) },
                "앞이 컴컴한 안경이다. 신비한 기운이 감싸고있다.", 2700)
        },
        {
            16,
            new Item(16, "요정의 깃털", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 25), new Stat(StatType.CriticalChance, 20) },
                "고대 요정의 깃털이다.", 3000)
        },
        {
            17,
        ...
        {
            18,
            new Item(18, "생명의 구슬", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 30), new Stat(StatType.MaxHp, 150) },
                "생명력이 가득 차있는 구슬이다.", 3500)
        },
        {
            19,
            new Item(19, "은밀 망토", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, -20), new Stat(StatType.CriticalChance, 60) },
                "겉으로 보기에는 변함이 없을 것이다", 5000)
        },
        {
            20,
            new Item(20, "변함 없는돌", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 50), new Stat(StatType.MaxHp, 100) },
                "겉으로 보기에는 변함이없다. 내용만 다를뿐", 6500)
        },
        {
            21,
            new Item(21, "학습 장치", ItemType.Armor,
                new List<Stat> { new Stat(StatType.Defense, 80), new Stat(StatType.Attack, 120) },
